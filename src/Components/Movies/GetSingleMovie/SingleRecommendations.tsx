...
const SingleRecommendations: FC<SingleMovieProps> = () => {
  ...
  const [acceptWorking, setAcceptWorking] = useState<boolean>(false);

  const [getSingleMovie, setGetSingleMovie] = useState<IResultData[]>();
  const { idR, setIdR } = useContext(MovieContext);

  useEffect(() => {
    let getSingleMovieR = recommendations?.results.filter((movie) => {
    ...
    if (getSingleMovieR) {
      setGetSingleMovie(getSingleMovieR);
    }
  }, [id, idR, !acceptWorking && recommendations]);
  useEffect(() => {
